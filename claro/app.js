...
function pushHistory(g,tipo){g.history=g.history||[];const last=g.history[g.history.length-1];if(last&&last.d===today){last.v=g.saved;last.t=tipo;}else if(!last||last.v!==g.saved){g.history.push({d:today,v:g.saved,t:tipo});}}
...
$('#simpleForm').addEventListener('submit',e=>{if(e.submitter?.value==='cancel')return;e.preventDefault();const kind=e.currentTarget.dataset.kind,ref=e.currentTarget.dataset.ref;
  ...
  else if(kind==='goal'){const ng={id:Date.now(),name:$('#simpleName').value,target:Number($('#simpleAmount').value),saved:Number($('#simpleSaved').value)||0,invested:($('#simpleInvested').value!==''?Number($('#simpleInvested').value):undefined),icon:$('#simpleIcon').value||'✦',currency:$('#simpleCur').value,place:$('#simplePlace').value.trim()};state.goals.push(ng);pushHistory(ng,'inicial');}
