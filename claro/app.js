...
$('#simpleForm').addEventListener('submit',e=>{if(e.submitter?.value==='cancel')return;e.preventDefault();const kind=e.currentTarget.dataset.kind,ref=e.currentTarget.dataset.ref;
  ...
  else if(kind==='goal'){state.goals.push({id:Date.now(),name:$('#simpleName').value,target:Number($('#simpleAmount').value),saved:Number($('#simpleSaved').value)||0,invested:($('#simpleInvested').value!==''?Number($('#simpleInvested').value):undefined),icon:$('#simpleIcon').value||'✦',currency:$('#simpleCur').value,place:$('#simplePlace').value.trim()});}
  else if(kind==='editGoal'){const g=state.goals.find(x=>x.id===Number(ref));if(g){g.name=$('#simpleName').value;g.target=Number($('#simpleAmount').value);g.saved=Number($('#simpleSaved').value)||0;g.invested=($('#simpleInvested').value!==''?Number($('#simpleInvested').value):undefined);g.icon=$('#simpleIcon').value||'✦';g.currency=$('#simpleCur').value;g.place=$('#simplePlace').value.trim();}}
