...
const moneyCur=(n,cur)=>{n=Number(n)||0;const hasCents=Math.abs(n-Math.round(n))>0.004;const d=hasCents?2:0;const s=new Intl.NumberFormat('es-AR',{minimumFractionDigits:d,maximumFractionDigits:d}).format(n);return cur==='USD'?('US$ '+s):('$ '+s);};
...
state.catFijo=state.catFijo||{}; // categorías marcadas como gasto fijo
...
function pushHistory(g,tipo,fecha){fecha=fecha||today;g.history=g.history||[];const ex=g.history.find(e=>e.d===fecha);if(ex){ex.v=g.saved;ex.t=tipo;}else{g.history.push({d:fecha,v:g.saved,t:tipo});}g.history.sort((a,b)=>a.d.localeCompare(b.d));}
function elapsedLabel(from){const a=new Date(from+'T12:00'),b=new Date(today+'T12:00');const days=Math.round((b-a)/86400000);if(days<=0)return 'hoy';if(days<31)return 'en '+days+' día'+(days===1?'':'s');let months=(b.getFullYear()-a.getFullYear())*12+(b.getMonth()-a.getMonth());if(b.getDate()<a.getDate())months--;if(months<12)return 'en '+months+' mes'+(months===1?'':'es');const y=Math.floor(months/12),m=months%12;return 'en '+y+' año'+(y===1?'':'s')+(m?' '+m+'m':'');}
function historyRows(g){const cur=g.currency||'ARS';const h=g.history||[];const lbl={inicial:'inicio',aporte:'aporte',ajuste:'ajuste'};let rows='';for(let i=h.length-1;i>=0;i--){const e=h[i],prev=h[i-1];let delta='';if(prev){const d=e.v-prev.v,pct=prev.v?Math.round(d/prev.v*100):0;delta=`<span class="${d>=0?'up':'down'}">${d>=0?'▲':'▼'} ${moneyCur(Math.abs(d),cur)} (${d>=0?'+':'−'}${Math.abs(pct)}%)</span>`;}rows+=`<div class="hist-row"><span class="hd">${new Date(e.d+'T12:00').toLocaleDateString('es-AR',{day:'numeric',month:'short',year:'2-digit'})}${e.t?' · '+(lbl[e.t]||e.t):''}</span><b>${moneyCur(e.v,cur)}</b>${delta}</div>`;}return rows;}
function renderGoals(){
  $('#goalList').innerHTML=state.goals.map(g=>{const cur=g.currency||'ARS';const hasTarget=Number(g.target)>0;const p=hasTarget?Math.min(100,g.saved/g.target*100):0;let gl='';if(typeof g.invested==='number'&&g.invested>0){const gain=g.saved-g.invested;const pct=Math.round(gain/g.invested*100);gl=`<p class="gl ${gain>=0?'up':'down'}">${gain>=0?'📈 Ganancia':'📉 Pérdida'}: ${gain>=0?'+':'−'}${moneyCur(Math.abs(gain),cur)} (${gain>=0?'+':''}${pct}%)${(g.history&&g.history.length)?' · '+elapsedLabel(g.history[0].d):''}</p>`;}return `<article class="goal-card"><div class="card-actions"><button class="mini-btn js-edit-goal" data-id="${g.id}">Editar</button><button class="mini-btn danger js-del-goal" data-id="${g.id}">Borrar</button></div><div class="goal-icon">${g.icon}</div><h2>${escapeHtml(g.name)} ${cur==='USD'?'<span class="cur-chip">US$</span>':''}</h2>${g.place?`<p class="goal-place">📍 ${escapeHtml(g.place)}</p>`:''}<div class="row"><strong>${moneyCur(g.saved,cur)}</strong>${hasTarget?`<span>de ${moneyCur(g.target,cur)}</span>`:'<span>hoy</span>'}</div>${gl}${hasTarget?`<div class="progress-track"><div class="progress-fill" style="width:${p}%;background:#a79ae5"></div></div><p class="remaining">Te faltan ${moneyCur(Math.max(0,g.target-g.saved),cur)} · ${Math.round(p)}% alcanzado</p>`:'<p class="remaining">Sin meta · vas sumando 💜</p>'}${(g.history&&g.history.length>1)?`<details class="hist"><summary>📊 Ver historial (${g.history.length})</summary><div class="hist-list">${historyRows(g)}</div></details>`:''}<button class="outline-button contribute" data-id="${g.id}">＋ Registrar aporte</button></article>`;}).join('')||'<p class="muted">Creá un objetivo para darle un nombre a tu ahorro.</p>';
  const at=$('#ahorroTotal');if(at){if(!state.goals.length){at.hidden=true;}else{const arsT=state.goals.filter(g=>(g.currency||'ARS')!=='USD').reduce((a,g)=>a+(Number(g.saved)||0),0);const usdT=state.goals.filter(g=>g.currency==='USD').reduce((a,g)=>a+(Number(g.saved)||0),0);at.hidden=false;at.innerHTML='<p class="eyebrow">TOTAL AHORRADO</p><div class="at-nums">'+(arsT||!usdT?'<strong>'+moneyCur(arsT,'ARS')+'</strong>':'')+(usdT?'<strong>'+moneyCur(usdT,'USD')+'</strong>':'')+'</div>';}}
}
...
function renderReports(){
  ...
  const gmes=monthMovements().filter(m=>m.type==='expense');const gFijos=gmes.filter(m=>state.catFijo[m.category]).reduce((a,m)=>a+m.amount,0);const gVar=gmes.filter(m=>!state.catFijo[m.category]).reduce((a,m)=>a+m.amount,0);const gTot=(gFijos+gVar)||1;const fv=$('#fixVarChart');if(fv)fv.innerHTML=(gFijos+gVar)?`<div class="fv-row"><span>🔁 Fijos</span><b>${currency(gFijos)}</b><span class="fv-pct">${Math.round(gFijos/gTot*100)}%</span></div><div class="progress-track"><div class="progress-fill" style="width:${gFijos/gTot*100}%;background:#88b7c6"></div></div><div class="fv-row" style="margin-top:12px"><span>🛒 Variables</span><b>${currency(gVar)}</b><span class="fv-pct">${Math.round(gVar/gTot*100)}%</span></div><div class="progress-track"><div class="progress-fill" style="width:${gVar/gTot*100}%;background:#e9aa85"></div></div>`:'<p class="muted">Todavía no hay gastos este mes.</p>';
...
function renderSettingsTags(){
  $('#categoryTags').innerHTML=state.categories.map(c=>`<button class="tag js-cat${state.catFijo[c]?' fijo':''}" data-name="${escapeHtml(c)}" title="${state.catFijo[c]?'Gasto fijo':'Gasto variable'}">${escapeHtml(c)}${state.catFijo[c]?' <span class="fx-dot">F</span>':''}</button>`).join('');
...
function openSimple(kind,ref=''){
  ...
  if(kind==='budget'){
    ...
    f.innerHTML='<label>Nombre<input id="simpleName" placeholder="Ej. Dólares, Plazo fijo, Viaje" value="'+(g0?escapeHtml(g0.name):'')+'" required></label>'
      ...
      +'<label>Meta <span style="font-weight:400">(opcional; dejala vacía si solo ahorrás)</span><input id="simpleAmount" type="number" min="0" step="any" inputmode="decimal" placeholder="—" value="'+(g0&&g0.target?g0.target:'')+'"></label>'
      +'<label>¿Cuánto tenés hoy? <span style="font-weight:400">(valor actual; no toca el banco)</span><input id="simpleSaved" type="number" min="0" step="any" inputmode="decimal" placeholder="0" value="'+(g0?g0.saved:'')+'"></label>'
      +'<label>¿Cuánto pusiste? <span style="font-weight:400">(capital, opcional — para ver si ganaste o perdiste)</span><input id="simpleInvested" type="number" min="0" step="any" inputmode="decimal" placeholder="—" value="'+(g0&&typeof g0.invested==='number'?g0.invested:'')+'"></label>'
      ...
      +'<label>Fecha de este valor <span style="font-weight:400">(para el historial y el rendimiento)</span><input id="simpleGoalDate" type="date" value="'+today+'"></label>';
    if(g0)$('#simpleCur').value=g0.currency||'ARS';
  }else if(kind==='category'){
    $('#simpleEyebrow').textContent='ORGANIZACIÓN';$('#simpleTitle').textContent='Nueva categoría';
    f.innerHTML='<label>Nombre de categoría<input id="simpleName" placeholder="Ej. Mascotas" required></label><label class="check-label"><input id="simpleCatFijo" type="checkbox"> Es un gasto fijo (servicios, seguros, tarjeta…)</label>';
  }else if(kind==='editCategory'){
    $('#simpleEyebrow').textContent='CATEGORÍA';$('#simpleTitle').textContent='Editar categoría';
    f.innerHTML=`<label>Nombre<input id="simpleName" value="${escapeHtml(ref)}" required></label><label class="check-label"><input id="simpleCatFijo" type="checkbox" ${state.catFijo[ref]?'checked':''}> Es un gasto fijo</label>`;
    ...
    if(gc==='USD'){f.innerHTML='<label>¿Cuántos dólares sumaste? <span style="font-weight:400">(US$)</span><input id="simpleAmount" type="number" min="0" step="any" inputmode="decimal" placeholder="0"></label>'+'<label>¿Cuántos pesos saliste del banco? <span style="font-weight:400">(opcional; si los compraste con plata de la cuenta)</span><input id="simpleFromBank" type="number" min="0" step="any" inputmode="decimal" placeholder="0"></label>'+'<label>Fecha <span style="font-weight:400">(cuándo)</span><input id="simpleGoalDate" type="date" value="'+today+'"></label>'+'<p class="small" style="color:var(--muted);margin-top:8px">Suma los dólares a la bolsita y descuenta del banco los pesos que pusiste.</p>';}
    else{f.innerHTML=fieldNumber('simpleAmount','¿Cuánto sumás?')+'<label>Fecha <span style="font-weight:400">(cuándo)</span><input id="simpleGoalDate" type="date" value="'+today+'"></label>'+'<p class="small" style="color:var(--muted);margin-top:8px">Sale de tu cuenta: descuenta de "En el banco".</p>';}
  }
  f.insertAdjacentHTML('beforeend',delBtn);
  $('#simpleDialog').showModal();setTimeout(()=>f.querySelector('input,select')?.focus(),50);
}

function renameCategory(oldName,newName){if(!newName||oldName===newName)return;if(state.catFijo[oldName]){state.catFijo[newName]=true;delete state.catFijo[oldName];}if(!state.categories.includes(newName))state.categories[state.categories.indexOf(oldName)]=newName;else state.categories=state.categories.filter(c=>c!==oldName);state.movements.forEach(m=>{if(m.category===oldName)m.category=newName;});state.budgets.forEach(b=>{if(b.category===oldName)b.category=newName;});state.fixed.forEach(fx=>{if(fx.category===oldName)fx.category=newName;});}
function deleteCategory(name){if(!state.categories.includes('Otros'))state.categories.push('Otros');delete state.catFijo[name];state.categories=state.categories.filter(c=>c!==name);state.movements.forEach(m=>{if(m.category===name)m.category='Otros';});state.budgets=state.budgets.filter(b=>b.category!==name);state.fixed.forEach(fx=>{if(fx.category===name)fx.category='Otros';});}
...
$('#simpleForm').addEventListener('submit',e=>{if(e.submitter?.value==='cancel')return;e.preventDefault();const kind=e.currentTarget.dataset.kind,ref=e.currentTarget.dataset.ref;
  ...
  else if(kind==='goal'){const ng={id:Date.now(),name:$('#simpleName').value,target:Number($('#simpleAmount').value),saved:Number($('#simpleSaved').value)||0,invested:($('#simpleInvested').value!==''?Number($('#simpleInvested').value):undefined),icon:$('#simpleIcon').value||'✦',currency:$('#simpleCur').value,place:$('#simplePlace').value.trim()};state.goals.push(ng);pushHistory(ng,'inicial',($('#simpleGoalDate')&&$('#simpleGoalDate').value)||today);}
  else if(kind==='editGoal'){const g=state.goals.find(x=>x.id===Number(ref));if(g){g.name=$('#simpleName').value;g.target=Number($('#simpleAmount').value);g.saved=Number($('#simpleSaved').value)||0;g.invested=($('#simpleInvested').value!==''?Number($('#simpleInvested').value):undefined);g.icon=$('#simpleIcon').value||'✦';g.currency=$('#simpleCur').value;g.place=$('#simplePlace').value.trim();pushHistory(g,'ajuste',($('#simpleGoalDate')&&$('#simpleGoalDate').value)||today);}}
  else if(kind==='category'){let n=$('#simpleName').value.trim();if(n){if(!state.categories.includes(n))state.categories.push(n);if($('#simpleCatFijo')&&$('#simpleCatFijo').checked)state.catFijo[n]=true;else delete state.catFijo[n];}}
  else if(kind==='editCategory'){const nn=$('#simpleName').value.trim();renameCategory(ref,nn);const fin=nn||ref;if($('#simpleCatFijo')&&$('#simpleCatFijo').checked)state.catFijo[fin]=true;else delete state.catFijo[fin];}
  ...
  else{let g=state.goals.find(g=>g.id===+kind),amount=Number($('#simpleAmount').value);const fAp=($('#simpleGoalDate')&&$('#simpleGoalDate').value)||today;if(g){g.saved+=amount;if(typeof g.invested==='number')g.invested+=amount;if((g.currency||'ARS')!=='USD'){state.movements.push({id:Date.now(),type:'saving',amount,description:g.name,category:'Ahorro',payment:state.payments[0],date:fAp,tags:[],professional:false});}else{const fromBank=Number($('#simpleFromBank')&&$('#simpleFromBank').value)||0;if(fromBank>0)state.movements.push({id:Date.now(),type:'saving',amount:fromBank,description:'Compra dólares · '+g.name,category:'Ahorro',payment:state.payments[0],date:fAp,tags:[],professional:false});}pushHistory(g,'aporte',fAp);}}
...
$('#replaceCats')?.addEventListener('click',()=>{const txt=$('#bulkCats').value||'';const names=[];const fijo={};txt.split(/\n/).forEach(l=>{const parts=l.split(/\t|\s{2,}/);const n=(parts[0]||'').trim();const lbl=(parts[1]||'').trim().toLowerCase();if(!n||/^(extras?|fijo|variable)$/i.test(n))return;if(!names.includes(n))names.push(n);if(/fij/.test(lbl))fijo[n]=true;});if(!names.length){showToast('Pegá primero la lista nueva arriba');return;}if(!confirm('Esto reemplaza TODAS tus categorías por la lista nueva ('+names.length+'). Tus movimientos guardados NO se borran. ¿Seguir?'))return;state.categories=names;state.catFijo=fijo;$('#bulkCats').value='';persist();showToast('Categorías reemplazadas: '+names.length);});
$('#addBulkCats')?.addEventListener('click',()=>{const txt=$('#bulkCats').value||'';let added=0,marcadas=0;txt.split(/\n/).forEach(l=>{const parts=l.split(/\t|\s{2,}/);const n=(parts[0]||'').trim();const lbl=(parts[1]||'').trim().toLowerCase();if(!n||/^(extras?|fijo|variable)$/i.test(n))return;if(!state.categories.includes(n)){state.categories.push(n);added++;}if(/fij/.test(lbl)){if(!state.catFijo[n]){state.catFijo[n]=true;marcadas++;}}else if(/var/.test(lbl)){delete state.catFijo[n];}});$('#bulkCats').value='';if(added||marcadas){persist();showToast('Agregué '+added+' categoría'+(added===1?'':'s')+(marcadas?' · '+marcadas+' fijas':''));}else showToast('No había nada nuevo para agregar');});
...
$('#exportGoals').onclick=()=>{if(!state.goals.length){showToast('Todavía no tenés ahorros para exportar');return;}let h=['Nombre','Moneda','Valor actual','Capital puesto','Ganancia/Pérdida','%','Dónde está','Meta','Desde','Última actualización'];let rows=state.goals.map(g=>{const cur=g.currency||'ARS';const cap=(typeof g.invested==='number')?g.invested:'';const gan=(typeof g.invested==='number')?Math.round((g.saved-g.invested)*100)/100:'';const pct=(typeof g.invested==='number'&&g.invested>0)?Math.round((g.saved-g.invested)/g.invested*100):'';const desde=(g.history&&g.history.length)?g.history[0].d:'';const ult=(g.history&&g.history.length)?g.history[g.history.length-1].d:'';return [g.name,cur,g.saved,cap,gan,pct,g.place||'',g.target||'',desde,ult];});download('﻿'+[h,...rows].map(r=>r.map(v=>'"'+String(v).replaceAll('"','""')+'"').join(';')).join('\n'),'claro-ahorros.csv','text/csv;charset=utf-8');showToast('Excel de ahorros descargado');};
